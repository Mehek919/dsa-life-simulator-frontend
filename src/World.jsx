...
function ActivityFeedPanel({ onClose }) {
  ...
  return (
    <motion.div
      ...
      className="fixed top-0 right-0 h-full w-80 bg-[#0d0d1f]/95 backdrop-blur-xl
          ...
          <AnimatePresence initial={false}>
            {events.map((ev) => {
              ...
              return (
                ...
                >
                  <div className="flex-shrink-0 w-7 h-7 rounded-full bg-white/10 flex items-center justify-center overflow-hidden">
                    {ev.photoURL
                      ? <img src={ev.photoURL} alt="av" className="w-full h-full object-cover rounded-full" />
...
function ZoneOrb({ zone, badgeCount, onClick }) {
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.96 }}
      className={`absolute ${zone.position} flex flex-col items-center gap-2 group`}
    >
      {/* Orb */}
      <div className={`relative w-20 h-20 rounded-full bg-gradient-to-br ${zone.color}
                       ${zone.glow} flex items-center justify-center
      ...
      <div className="text-center">
        ...
        <p className="text-[10px] text-gray-400 group-hover:text-gray-200 transition-colors">
          {zone.desc}
        </p>
      </div>
    </motion.button>
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        ...
        const unattempted = list.filter(
          (ch) => !ch.attemptedBy?.includes(user.uid)
        ).length;
        setHubCount(unattempted);
      })
      .catch(() => setHubCount(0));
  }, [user?.uid]);

  // ── feed preview (top 4 events) ────────────────────────────────────────────
  useEffect(() => {
    const q = query(
      collection(db, 'activityFeed'),
      orderBy('createdAt', 'desc'),
  ...
  return (
    <div className="relative min-h-screen bg-[#060612] text-white overflow-hidden font-mono select-none">
      ...
      <div className="relative z-10 flex items-center justify-between px-6 pt-6 pb-2">
        {/* Left: title + level */}
        <div>
          <h1 className="text-xl font-bold text-cyan-400 tracking-widest">DSA WORLD</h1>
      ...
      {/* ── XP bar mobile ── */}
      <div className="md:hidden px-6 mt-1 relative z-10">
      ...
      {!showFeed && !feedLoading && feedPreview.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 z-20 bg-black/60 backdrop-blur-sm
          ...
          <span className="text-cyan-400 font-bold flex-shrink-0">📡 Live</span>
