...
// ── Proctoring Hook ────────────────────────────────────────────────────────────
function useProctoring({ enabled, assessmentId, userId, onViolation }) {
  ...
  useEffect(() => {
    ...
    const handleVisibility = () => {
      if (document.hidden) {
        tabSwitches.current++;
        logViolation('tab_switch', `Tab switch #${tabSwitches.current}`);
      }
    };
    ...
    // Dev tools (F12, Ctrl+Shift+I)
    const handleKeyDown = (e) => {
      if (e.key === 'F12' || (e.ctrlKey && e.shiftKey && e.key === 'I')) {
        e.preventDefault();
        logViolation('devtools_attempt');
      }
      if (e.ctrlKey && e.key === 'c') logViolation('copy', 'Ctrl+C');
      if (e.ctrlKey && e.key === 'v') logViolation('paste', 'Ctrl+V');
    };
  ...
  return { violations: violations.current, tabSwitches: tabSwitches.current };
}
...
function AssessmentTimer({ totalSeconds, onExpire }) {
  ...
  return (
    <div style={{
      ...
      borderRadius: 10,
      padding:      '8px 16px',
      boxShadow:    isLow ? `0 0 16px ${color}33` : 'none',
    }}>
      ...
      <div>
        <motion.div
          ...
          style={{ color, fontSize: 20, fontWeight: 900, fontFamily: 'monospace' }}
        >
...
function ViolationBanner({ violation, onDismiss }) {
  ...
  const messages = {
    tab_switch:       '⚠️ Tab switch detected — this is being logged.',
    copy:             '⚠️ Copy detected — assessment is proctored.',
    paste:            '⚠️ Paste detected — assessment is proctored.',
    right_click:      '⚠️ Right-click is disabled during assessment.',
    devtools_attempt: '⚠️ DevTools are disabled during assessment.',
  };
...
function ProblemNav({ problems, current, solved, scores, onSelect }) {
  return (
    <div style={{
      ...
      <div style={{ color: '#555', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8, padding: '0 4px' }}>
        ...
        return (
          <button
            ...
            style={{
              ...
              gap:          10,
              padding:      '10px 12px',
              background:   isCurrent ? '#1a73e811' : isSolved ? '#00c89608' : 'transparent',
              border:       `1px solid ${isCurrent ? '#1a73e844' : isSolved ? '#00c89633' : '#1e2a3a'}`,
              borderRadius: 10,
              cursor:       'pointer',
              textAlign:    'left',
              transition:   'all 0.2s',
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{
                ...
                color:        isCurrent ? '#e8e8e8' : '#c8c8c8',
              }}>
                {p.title}
              </div>
              <div style={{ display: 'flex', gap: 6, marginTop: 2, alignItems: 'center' }}>
                <span style={{ color: diffColor, fontSize: 9, fontWeight: 700 }}>{p.difficulty}</span>
                {isSolved && scores[p.id] && (
                  <span style={{ color: '#00c896', fontSize: 9 }}>+{scores[p.id]}</span>
                )}
              </div>
            </div>
          </button>
    ...
    <div style={{
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
          ...
          <div>
            <div style={{ color: '#1a73e8', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
        ...
        <h2 style={{ margin: '0 0 8px', color: '#e8e8e8', fontSize: 20, fontWeight: 700 }}>
          {assessment.title}
        </h2>
        <p style={{ margin: '0 0 24px', color: '#666', fontSize: 14, lineHeight: 1.6 }}>
          {assessment.description || 'Complete all problems within the time limit.'}
        </p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 24 }}>
          {[
            ...
            { icon: '💻', label: 'Languages', value: assessment.allowedLanguages?.join(', ') || 'All languages' },
            { icon: '🔒', label: 'Proctored', value: assessment.proctored ? 'Yes — activity logged' : 'No' },
          ].map(d => (
            <div key={d.label} style={{
              background: '#060910', border: '1px solid #1e2a3a',
              borderRadius: 10, padding: '10px 14px',
            }}>
              <div style={{ color: '#555', fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 3 }}>
                {d.icon} {d.label}
              </div>
        ...
        <div style={{
          ...
          <div style={{ color: '#ff6b6b', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>
            ⚠️ Rules
          </div>
          <ul style={{ margin: 0, padding: '0 0 0 16px', color: '#888', fontSize: 12, lineHeight: 1.8 }}>
            ...
            <li>Write your own code — no AI assistance</li>
            <li>All activity is logged and reviewed by the company</li>
        ...
        <label style={{
          ...
          />
          I agree to the assessment rules. I understand this is a proctored environment and my activity will be logged and shared with {assessment.companyName || 'the company'}.
        </label>
        ...
        <motion.button
          ...
          style={{
            ...
            border:       'none',
            borderRadius: 12,
            color:        agreed ? '#fff' : '#444',
            cursor:       agreed ? 'pointer' : 'not-allowed',
            fontSize:     15,
            fontWeight:   800,
            padding:      '14px 0',
            boxShadow:    agreed ? '0 0 24px #1a73e833' : 'none',
            transition:   'all 0.3s',
          }}
        >
    ...
    <div style={{
      ...
      >
        ...
        <p style={{ color: '#666', margin: '0 0 28px', fontSize: 14 }}>
          Your results have been submitted to {assessment.companyName || 'the company'}.
        </p>
        ...
        <div style={{ marginBottom: 28, position: 'relative', display: 'inline-block' }}>
          <svg width={120} height={120} style={{ transform: 'rotate(-90deg)' }}>
            ...
            <motion.circle
              ...
              stroke={pct >= 70 ? '#00c896' : pct >= 40 ? '#f5c542' : '#ff4d4d'}
              strokeWidth={8} strokeLinecap="round"
              strokeDasharray={314}
              initial={{ strokeDashoffset: 314 }}
              animate={{ strokeDashoffset: 314 * (1 - pct / 100) }}
        ...
        {violations?.length > 0 && (
          <div style={{
            ...
            <div style={{ color: '#ff6b6b', fontSize: 11, fontWeight: 700, marginBottom: 4 }}>
              ⚠️ {violations.length} activity flag{violations.length !== 1 ? 's' : ''} recorded
            </div>
            <div style={{ color: '#666', fontSize: 11 }}>
              These have been included in your submission report.
            </div>
          </div>
        )}

        <p style={{ color: '#555', fontSize: 13, margin: '0 0 24px' }}>
          The hiring team will review your submission and get back to you within 3-5 business days.
        </p>
...
export default function AssessmentPortal({ user, userData, setUserData }) {
  ...
  const handleSubmit = async (code, langId, testResults) => {
    ...
    try {
      ...
      if (allPassed) {
        ...
        const nextUnsolved = assessment.problems.findIndex(
          (p, i) => i > currentIdx && !solved.includes(p.id)
        );
        if (nextUnsolved !== -1) setTimeout(() => setCurrentIdx(nextUnsolved), 1000);
      } else {
        showToast(`❌ ${passed}/${total} tests passed — keep trying`, 'error');
      }
  ...
  const handleTimeExpire = useCallback(async () => {
    ...
    try {
      await axios.post(`${API_BASE}/assessments/${assessmentId}/complete`, {
        userId:     user?.uid,
        solved,
        scores,
        violations,
  ...
  if (phase === 'intro') {
    return <AssessmentIntro assessment={assessment || {}} onStart={handleStart} />;
  }
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            <div style={{ color: '#1a73e8', fontSize: 10, fontWeight: 700, textTransform: 'uppercase' }}>
              {assessment?.companyName} — Technical Assessment
            </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{
            ...
            <div style={{ color: '#a855f7', fontSize: 16, fontWeight: 900 }}>
          ...
          {assessment?.proctored && (
            <div style={{
              ...
              <span style={{ color: '#ff6b6b', fontSize: 11, fontWeight: 600 }}>
                Proctored · {violations.length} flag{violations.length !== 1 ? 's' : ''}
              </span>
            </div>
          )}
          ...
          {/* Timer */}
          <AssessmentTimer
            totalSeconds={assessment?.durationMinutes * 60 || 3600}
        ...
        <div style={{ flex: 1, overflow: 'hidden' }}>
          {currentProblem && (
            <CodeEditor
              ...
              defaultLanguage={assessment?.defaultLanguage || 'python3'}
            />
          )}
        </div>
      </div>
