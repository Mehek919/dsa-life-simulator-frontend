      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {track.problems.map((slug, i) => {
            const isSolved = userProgress?.solvedProblems?.includes(slug);
            return (
              <div
                key={slug}
...
export default function Roadmap({ user, userData }) {
  ...
  const getTrackProgress = (track) => {
    const solved = (userData?.solvedProblems || []).filter(id => track.problems.includes(id)).length;
